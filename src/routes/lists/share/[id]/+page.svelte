<script lang="ts">
	...
	const sharing = async () => {
		try {
			...
			const response = await axios.post('http://localhost:5173/api/lists/share/' + listId, data, {
				withCredentials: true
			});
			if (response.status === 200) {
				goto('/lists')
		...
		<div class="container max-w-lg mx-auto flex-1 flex flex-col items-center justify-center px-2">
			<div class="bg-stone-50 px-6 py-10 rounded shadow-lg h-[160] text-black w-full rounded-xl">
				...
				<input
					...
					placeholder="Megosztani való fiók email címe"
				/>
				<button
					type="submit"
					...
					on:click={sharing}>Megosztás</button
				>
			</div>
		</div>
	</div>
